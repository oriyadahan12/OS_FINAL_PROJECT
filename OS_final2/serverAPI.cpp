...
void initGraph(Graph *g, int m, int fd_client){
    std::string msg = "enter edge: u v w \n";
    if (send(fd_client, msg.c_str(), msg.size(), 0) < 0){
        perror("send"); // Handle send error
    }
    int stdin_save = dup(STDIN_FILENO); // Save the current state of STDIN
    dup2(fd_client, STDIN_FILENO);       // Redirect STDIN to the socket
    for (int i = 0; i < m; i++){
        // Read the edges
        size_t u, v, weight;
